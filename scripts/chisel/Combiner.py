...
def parse_args(args):
    ...
    parser.add_argument(
        ...
        type=str,
        default="50kb",
        help="Size of the haplotype blocks (default: 50kb, use 0 to disable)",
    )
    parser.add_argument(
        "-a",
        "--significance",
        ...
        type=float,
        default=0.05,
        help="Significance level to estimate the maximum shift error for BAF = 0.5 (default: 0.05)",
    )
    parser.add_argument(
        "-q",
        "--restarts",
        ...
        type=int,
        default=100,
        help="Number of restarts for EM (default: 100)",
    )
    parser.add_argument(
        "-t",
        "--bootstrap",
        ...
        type=int,
        default=100,
        help="Number of bootastrapping points to estimate maximum shift error (default: 100)",
    )
    parser.add_argument(
        "-e",
        "--maxerror",
        ...
        type=float,
        default=None,
        help="Maximum shift error for identification of BAF = 0.5 (default: not used, when specified the value is used instead of estimation)",
    )
    parser.add_argument(
        "-E",
        "--minerror",
        ...
        type=float,
        default=0.001,
        help="Minimum shift error for identification of BAF = 0.5 (default: 0.001)",
    )
    parser.add_argument(
        "-l",
        "--listofcells",
        ...
        type=str,
        default=None,
        help="List of cells to include (default: None)",
    )
    parser.add_argument(
        "-s",
        "--seed",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Minimum SNP density per kb (default: 0.0, reasonable values are 0.01, 0.02, etc.)",
    )
    parser.add_argument(
        "--missingsnps",
        required=False,
        type=str,
        default=None,
        help="A,B counts for genomic bins without minimum minimum SNP density (default: 0,0 i.e. BAF=0.5)",
    )
    parser.add_argument(
        "--gccorr",
        required=False,
        type=str,
        default=None,
        help="The reference genome to apply additional GC correction in addition to using a matched-normal sample (default: None)",
    )
    parser.add_argument(
        "--alphagc",
        required=False,
        type=float,
        default=0.05,
        help="Significance level to decide GC correction (default: 0.05)",
    )
    parser.add_argument(
        "--nophasecorr",
        required=False,
        default=False,
        action="store_true",
        help="Disable correction for given phasing bias (default: enabled)",
    )
    parser.add_argument("--allele-swap-file", required=False, default=None)
    ...
    return {
        ...
        "allele_swap_file": args.allele_swap_file,
    }
...
def main(args=None, stdout_file=None):
    ...
    log("Combining")
    rb, isbalanced, swaps = combo(rdr, cA, cB, bulk, args)

    if args["gccorr"]:
        log("Applying additional GC correction")
        rdr = addgccorrect(rb, isbalanced, args)

    stdout_f = open(args["out_file"], "w")

    log("Printing combined RDR and BAF")
    baf = lambda A, B: (float(B) / float(A + B)) if A + B > 0 else 0.5
    gen = ((c, b, e) for c in sorted(rb) for b in sorted(rb[c], key=(lambda x: x[0])) for e in sorted(rb[c][b]))
    for c, b, e in gen:
        formb = lambda c, b, e: [
            rb[c][b][e]["BAF"][0],
            rb[c][b][e]["BAF"][1],
    ...
    if args["allele_swap_file"] is not None:
        swap_df = pd.DataFrame(swaps)

        if swap_df.empty:
            open(args["allele_swap_file"], "w").close()
        else:
            swap_df = swap_df.sort_values([0, 1, 2, 3])

            swap_df.to_csv(args["allele_swap_file"], header=False, index=False, sep="\t")
...
def read_baf(f, cells):
    ...
    return (
        {c: dict(countA[c]) for c in countA},
        {c: dict(countB[c]) for c in countB},
        {c: dict(bulk[c]) for c in bulk},
    )
...
def combo(rdr, cA, cB, bulk, args):
    ...
    allele_swaps = []
    for c, b, A, B, isbal, swaps in pool.imap_unordered(combine, jobs):
        ...
        allele_swaps.extend([[c, b[0], b[1], o, int(swaps[o])] for o in sorted(swaps)])
    pool.close()
    pool.join()
    return rb, isbalanced, allele_swaps
...
def init(_snps, _cA, _cB, _bulk, _cells, args):
    ...
    alpha_correct = args["significance"] / max(20 * float(sum(len(snps[c]) for c in snps)), 1)
    maxerror = args["maxerror"]
    minerror = args["minerror"]
    minimumsnps = args["minimumsnps"]
...
def combine(job):
    ...
    if L >= R or snpdensity < minimumsnps:
        if missingsnps is None:
            return c, b, dict(), dict(), False, dict()
        else:
            return (
                c,
                b,
                {e: missingsnps[0] for e in cells},
                {e: missingsnps[1] for e in cells},
                False,
                {o: 0 for o in snps[c]},
            )
    ...
    def count_block(block):
        if phasecorr:
            ...
            assert all(p < q for p, q in zip(block[:-1], block[1:])), "Positions in block {} are not sorted!".format(
                block
            )
            backward = {
                p: (
                    sum(bulk[c][q][0] for q in block[: x + 1]),
                    sum(bulk[c][q][1] for q in block[: x + 1]),
                )
                for x, p in enumerate(block[:-1])
            }
            forward = {
                p: (
                    sum(bulk[c][q][0] for q in block[x + 1 :]),
            ...
            dotest = lambda p, flip: (
                sm.stats.proportions_ztest(
                    count=(backward[p][0], forward[p][0 if not flip else 1]),
            ...
            if sum(flips) > 0:
                sel, _ = max(
                    ((x, abs(s)) for x, s in enumerate(tstat) if flips[x]),
                    key=(lambda p: p[1]),
                )
            else:
                sel = len(block) + 1
        ...
        return (
            sum(bulk[c][p][0 if not dofix[p] else 1] for p in block),
    ...
    beta = max(
        (EM(ns=nis, xs=xis, start=np.random.randint(low=1, high=49) / 100.0) for r in xrange(restarts)),
        key=(lambda x: x[1]),
    )[0]
    if maxerror is None:
        thres = max(
            minerror,
            est_error(ns=nis, significance=alpha, restarts=restarts, bootstrap=boot),
        )
    else:
        thres = maxerror
...
def est_error(ns, significance=0.05, restarts=50, bootstrap=100):
    ...
    runEM = lambda xis: max(
        (EM(ns=nis, xs=xis, start=genstart()) for x in xrange(restarts)),
        key=(lambda x: x[1]),
    )[0]
...
def gccorrecting(e):
    getgc = lambda C: (
        ((C["C"] + C["G"]) / float(C["C"] + C["G"] + C["A"] + C["T"]))
        if (C["C"] + C["G"] + C["A"] + C["T"]) > 0
        else 0.5
    )
    safediv = lambda n, t: n / t if t > 0.0 else 1.0
    norm = safediv(
        sum(data[c][b][e]["normalcount"] for c in data for b in data[c]),
        float(sum(data[c][b][e]["readcount"] for c in data for b in data[c])),
    )
    form = lambda c, b: {
        "Tumor": data[c][b][e]["readcount"],
...
def gccorr(curr, alphagc, rkey="Tumor", plot=False, frac=1.0, tol=0.2, genomethres=0.4):
    ...
    xs1, ys1 = map(
        np.array,
        zip(*((curr[b]["%GC"], curr[b][rkey]) for b in sel1 if min1 <= curr[b][rkey] <= max1)),
    )
    ys1 = reg(ys1)
    id1 = ys1 >= tol
    if not any(id1):
